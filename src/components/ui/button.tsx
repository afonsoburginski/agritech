...
if (!isExpoGo) {
  try {
    // Usar import dinâmico para evitar que Metro tente resolver em tempo de build
    ...
    // Reanimated não disponível - usar View como fallback
    ...
    if (!error?.message?.includes('Cannot find native module') && 
      ...
      console.warn('Reanimated não disponível:', error?.message);
    }
  }
}
  ...
  (
    {
      ...
      disabled = false,
      loading = false,
      animation = !isExpoGo, // Desabilitar animações no Expo Go
      haptic = true,
    ...
    // Animation values for liquid glass effect (apenas se reanimated disponível)
    const scale = useSharedValue ? useSharedValue(1) : { value: 1 };
    const brightness = useSharedValue ? useSharedValue(1) : { value: 1 };
    ...
    // Animated styles using useAnimatedStyle (apenas se disponível)
