...
export const Input = forwardRef<TextInput, InputProps>(
  (
    ...
    // Placeholder color com melhor contraste: mais escuro no light, mais claro no dark
    const placeholderColor = useColor(
      { light: '#6B7280', dark: '#9CA3AF' }, // Cinza escuro no light, cinza médio no dark
      'textMuted'
    );

    const isTextarea = type === 'textarea';
    
    // Extrair borderRadius do containerStyle se fornecido
    const customBorderRadius = containerStyle?.borderRadius;

    // Placeholder color: na variante outline sempre branco, senão usa cor do tema
    const placeholderColorForVariant = variant === 'outline' 
      ? '#FFFFFF' 
      : placeholderColor;
    ...
    const getVariantStyle = (): ViewStyle => {
      // Se containerStyle tiver borderRadius, usar ele; senão usar o padrão
      const baseStyle: ViewStyle = {
        borderRadius: customBorderRadius ?? (isTextarea ? BORDER_RADIUS : CORNERS),
      ...
      switch (variant) {
        case 'outline':
          return {
            ...baseStyle,
            borderWidth: 1,
            // Borda sempre branca na variante outline
            borderColor: error ? danger : isFocused ? '#FFFFFF' : '#FFFFFF',
        ...
        case 'secondary':
          return {
            ...baseStyle,
            borderWidth: 1,
            // Borda transparente quando não focado, foreground quando focado
            borderColor: error ? danger : isFocused ? foreground : 'transparent',
            // Fundo secondary: usa secondary color do tema
            backgroundColor: disabled ? muted + '20' : secondaryColor,
          };
        case 'default':
        default:
          return {
            ...
            // Borda transparente quando não focado, foreground quando focado
            borderColor: error ? danger : isFocused ? foreground : 'transparent',
            // Fundo default: usa card que é cinza claro (#F2F2F7) no light, escuro no dark
    ...
    const getInputStyle = (): TextStyle => {
      // Na variante outline, texto sempre branco
      const textColorForVariant = variant === 'outline' ? '#FFFFFF' : (disabled ? muted : error ? danger : textColor);
    ...
    const renderInputContent = () => {
      // Separar borderRadius do containerStyle para aplicar no Pressable
      const { borderRadius, ...restContainerStyle } = containerStyle || {};
      
      return (
        <View style={restContainerStyle}>
          {/* Input Container */}
          <Pressable
        ...
        >
          {isTextarea ? (
            ...
            <>
              ...
              <View pointerEvents={disabled ? 'none' : 'auto'}>
                <TextInput
                  ...
                  placeholderTextColor={error ? danger + '99' : placeholderColorForVariant}
            ...
            >
              {/* Left section - Icon + Label (only if exists) */}
              {(icon || label) && (
                <View
                  style={{
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <TextInput
                  ...
                  placeholder={placeholder}
                  placeholderTextColor={error ? danger + '99' : placeholderColor}
                  editable={!disabled}
                  selectionColor={foreground}
                  onFocus={handleFocus}
                  onBlur={handleBlur}
