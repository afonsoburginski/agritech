...
export default function HomeScreen() {
  ...
  const { activities, isLoading: activitiesLoading, refresh: refreshActivities } = useSupabaseActivities();
  const { scouts, isLoading: scoutsLoading, refresh: refreshScouts } = useSupabaseScouts();
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      refreshActivities();
      refreshScouts();
    }, [refreshStats, refreshActivities, refreshScouts])
  );
  ...
  // Dados para radar chart - usar valores absolutos com mínimo de 10 para visualização
  const atividadesPorMesRadar = activitiesByMonth.map(item => ({
    label: item.label,
    value: Math.max(item.value * 10, item.value > 0 ? 20 : 5), // Escalar para melhor visualização
  }));
                ...
                >
                  <RadarChart
                    ...
                    config={{
                      ...
                      animated: true,
                      duration: 1000,
                      maxValue: Math.max(...atividadesPorMesRadar.map(a => a.value)) * 1.2,
                    }}
                  />
                </ChartContainer>
              </Card>
                  ...
                  atividadesRecentes.map((atividade) => (
                    <Card key={atividade.id} style={styles.activityCard}>
                      <View style={styles.activityHeader}>
                        ...
                        <Badge variant={atividade.situacao === 'CONCLUIDA' ? 'success' : atividade.situacao === 'EM_ANDAMENTO' ? 'default' : 'outline'}>
                          {atividade.situacao === 'CONCLUIDA' ? 'Concluída' : atividade.situacao === 'EM_ANDAMENTO' ? 'Em Andamento' : 'Pendente'}
                        </Badge>
                      </View>
                      {atividade.descricao && (
                      ...
                      <View style={styles.scoutHeader}>
                        <View style={styles.scoutLocation}>
                          ...
                          <Text variant="body" style={{ color: textColor, marginLeft: 6, flex: 1 }} numberOfLines={1}>
                            {scout.talhaoNome || scout.nome}
                          </Text>
                        </View>
                        <Badge variant={scout.status === 'CONCLUIDO' ? 'success' : 'outline'}>
                          {scout.status === 'CONCLUIDO' ? 'Concluído' : scout.status === 'EM_ANDAMENTO' ? 'Em Andamento' : 'Pendente'}
                        </Badge>
                      </View>
                      {scout.totalPragas > 0 && (
                        <View style={styles.scoutInfo}>
                          ...
                          <Text variant="caption" style={{ color: mutedColor, marginLeft: 4 }}>
                            {scout.totalPragas} {scout.totalPragas === 1 ? 'praga' : 'pragas'} detectada{scout.totalPragas > 1 ? 's' : ''}
                          </Text>
                        </View>
                      )}
                      {scout.observacao && (
